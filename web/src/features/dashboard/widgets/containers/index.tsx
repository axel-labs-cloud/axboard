...
function RestartButton({ busy, onClick }: { busy: boolean; onClick: (e: React.MouseEvent) => void }) {
  return (
    <button
      ...
      className={`w-5 h-5 shrink-0 flex items-center justify-center rounded text-text-muted hover:text-accent hover:bg-accent/10 transition-opacity ${busy ? "opacity-100" : "opacity-0 group-hover/c:opacity-100 focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60"}`}
    >
